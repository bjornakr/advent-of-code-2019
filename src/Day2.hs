module Day2 where
  ...
  execMath op p@(Program opCodePos intcodes) =
    let
      vs = getValues p
      res = uncurry op vs
    in
  ...
  solve = runInit $ replace 2 2 (replace 1 12 realInput)
  ...
  p1 = Program 0 [1,9,10,3,2,3,11,0,99,30,40,50]
